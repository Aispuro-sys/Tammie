...
(function(root){
	...
	stuQuery.prototype.ajax = function(url,attrs){
		...
		if(responseTypeAware && attrs.dataType){
			try { 
                // PATCH: Only set responseType if it's a valid XHR response type
                if(['arraybuffer','blob','document','json','text'].indexOf(attrs.dataType) >= 0) {
                    oReq.responseType = attrs.dataType; 
                }
            }
			catch(err){ error(err); }
		}
